...
```mermaid
graph LR
    A[Pregunta del Usuario] --> B[Motor IA de Consensus]
    B --> C[220M+ Artículos Científicos]
    C --> D[Síntesis con Evidencia]
    D --> E[Respuesta con Citas]
```
...
```mermaid
graph TD
    A[Página de Inicio] --> B[Barra de Búsqueda Central]
    A --> C[Menú Superior]
    C --> D[Research Hub]
    C --> E[Historial]
    C --> F[Configuración]
    B --> G[Página de Resultados]
    G --> H[Panel Izquierdo: Filtros]
    G --> I[Panel Central: Resultados]
    G --> J[Panel Derecho: Detalles]
```
